package com.ofbiz.importinterface.services.category;
...
public class ImportCategoryServices {
	public static final String module = ImportCategoryServices.class.getName();
	private static Map<String, String> categoryMap;
	private static List<Map<String, String>> categoryMapList;
	private static List<String> categoryProcessedList;
	...
	private static void parseCategoryFromMapList(String fileName) throws IOException, CategoryImportException {
		...
		categoryProcessedList = new ArrayList();
		int size = categoryMapList.size();
		for (int i = 0; i < size; i++) {
			try {
				categoryMap = categoryMapList.get(i);
				if(!categoryProcessedList.contains(categoryMap.get(CategoryXML.Category_Id.name()) + "_" + categoryMap.get(CategoryXML.Category_Type.name()) + "_" + categoryMap.get(CategoryXML.Parent_Category_ID.name()))) {
					categoryProcessedList.add(categoryMap.get(CategoryXML.Category_Id.name()) + "_" + categoryMap.get(CategoryXML.Category_Type.name()) + "_" + categoryMap.get(CategoryXML.Parent_Category_ID.name()));
	...
	private static List<Element> getProductCategoryAndRollup() throws CategoryImportException {
		...
		productCategory.setAttribute(new Attribute(CategoryXML.Category_Type.getOfbizColName(), categoryMap.get(CategoryXML.Category_Type.name())));
		...
		if(UtilValidate.isNotEmpty(categoryMap.get(CategoryXML.Category_Id.name())) && UtilValidate.isNotEmpty(categoryMap.get(CategoryXML.Parent_Category_ID.name()))) {
			...
			productCategoryRollup.setAttribute(new Attribute(CategoryXML.From_Date.getOfbizColName(), nowTimestamp));
	...
	public static Map<String, Object> importCategoryInERPFromCsv(DispatchContext ctx, Map<String, ?> context)
		...
		try {
			...
			Map prepareAndImportCategoryXMLResult = dispatcher.runSync("prepareAndImportCategoryXML",UtilMisc.toMap("userLogin", context.get("userLogin")));
			...
			Debug.logInfo("----- prepareAndImportCategoryXMLResult : "+prepareAndImportCategoryXMLResult+" -----outputXmlFilePath : "+outputXmlFilePath, module);
