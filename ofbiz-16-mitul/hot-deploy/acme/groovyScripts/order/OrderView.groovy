...
println("======202020202======================="+jobCardId+"=================================");
println("======202020202==========orderId============="+parameters.orderId+"=================================");
if(UtilValidate.isEmpty(orderId)){
    orderHeader = from("OrderHeader").where("jobCardId", jobCardId,"ownerId",userLogin.ownerCompanyId).queryOne()
    if(orderHeader){
        orderId = orderHeader.orderId;
    }
}

println("======202020202======================="+orderId+"=================================");
if(orderId){
println("====2222222===orderId======================="+orderId+"=================================");
//picklistBinList = from("OrderHeaderAndItems").where("orderId", orderId).queryList()

   picklistBinList = from("PicklistBin").where("primaryOrderId", orderId,"ownerId",userLogin.ownerCompanyId).queryList()
   println("252555252525=======picklistBinList======================="+picklistBinList+"=================================");
   context.picklistBinList = picklistBinList
