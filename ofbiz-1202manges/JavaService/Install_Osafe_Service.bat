...
set OFBIZ_HOME=E:\bigfish-project\ofbiz-1202manges
...
javaservice -install "Manges" %JAVA_HOME%\jre\bin\server\jvm.dll -Xms512M -Xmx512M %SOLR_PARMS% -Djava.class.path=%JAVA_HOME%\lib\tools.jar;%OFBIZ_HOME%\ofbiz.jar -start org.ofbiz.base.start.Start -out %LOGS_DIR%\serviceLog.txt -err %LOGS_DIR%\serviceErr.txt -current %OFBIZ_HOME% -manual
...
@echo .
@echo . Failed to install Manges as a system service
@echo .
